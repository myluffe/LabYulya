...
{
	char* number = (char*)heap_my.get_mem(sizeof(char) * ((int)strlen(str) + 1));
...
char * Parser::IntToString(int value)
{
	...
	char* str = (char*)heap_my.get_mem(sizeof(char) * (count + 1) );
	char* res = (char*)_itoa_s(value, str, count, 10);
	heap_my.free_mem(str);
	return res;
}
...
{
	char* temp = (char*)heap_my.get_mem(sizeof(char) * (6));
	if (value)
